...
describe('run agent', () => {
  ...
  it('relaunches in hint mode after the user skipped the provider step in setup', async () => {
    ...
    expect(captured[0]?.runtimeExtension).toContain('const OUTFITTER_PROVIDER_PROMPT_MODE = "hint";');
    ...
    expect(captured[1]?.runtimeExtension).toContain('const OUTFITTER_PROVIDER_PROMPT_MODE = "dialog";');
  });
