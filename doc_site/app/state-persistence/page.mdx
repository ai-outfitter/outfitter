...
description: How ApplePi handles agent state writes inside temporary composite profiles
---

# State Persistence

ApplePi creates a temporary runtime directory, called a **composite profile**, each time it launches an agent profile. The composite profile should be disposable, but agent CLIs also write durable state such as logins, settings, plugins, caches, and sessions.

ApplePi makes those writes explicit with `state_persistence` rules. Each selected adapter declares the state paths it knows about and the strategies each path allows, then the resolved profile can override paths with an allowed strategy such as persisting, discarding, warning, or failing after the run.
...
Update scenario: you run `applepi run --profile engineering`, complete a provider login, and pi writes `auth.json`. Because the path is symlinked, the login lands in the profile-provided or native pi state location rather than disappearing with the composite profile.
...
Update scenario: a CI run unexpectedly edits `mcp.json` or creates an undeclared state file. The agent process can finish, but ApplePi fails afterward because the profile declared those changes unacceptable.
...
Update scenario: you experiment with plugin installation during a run. ApplePi lets the agent continue, discards the temporary plugin changes afterward, and reports what changed so you can decide whether to promote it into a profile.
...
Update scenario: pi writes session transcripts and cache entries while investigating a problem. Those files help during the run, but ApplePi deletes them with the composite profile so the profile stays clean.
