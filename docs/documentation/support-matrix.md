...
- **Agent identity and appended prompts** — Codex launches do not project the composed agent identity/system prompt or documents supplied through `--append-prompt`. Identity is reported as an unsupported element on every Codex launch; supplied append-prompt documents produce an additional warning. There is no native identity flag projection yet, so `--strict` aborts every Codex launch before execution.
...
- **MCP servers (Partial)** — selected stdio fields (`command`, `args`, `env`, `cwd`) and streamable HTTP fields (`url`, `headers`) become repeated TOML-valued `-c mcp_servers.<id>.<key>=...` overrides. Legacy SSE and other HTTP transport types are unsupported and warn instead of being projected. Codex merges these overrides with user and project `config.toml`; Codex has no way to run with only the projected MCP servers, so Outfitter emits an additional additive-projection warning on every MCP-enabled Codex launch.
...
- **HTTP header safety** — a header value written as `${ENV_NAME}` becomes an `env_http_headers` reference, while `Authorization: Bearer ${ENV_NAME}` becomes `bearer_token_env_var`, keeping secret values out of argv. Other header values are literal protocol configuration and must be passed through `http_headers`, which makes them visible in process arguments; Outfitter emits a warning naming each exposed entry, so use environment references for secrets.
