...
¹ Canonical `anthropic-messages` providers. ² Canonical `openai-responses` providers. Other dialects, including `openai-completions`, warn and fail under `--strict` rather than changing endpoints.
...
- **Model selection** — an agent's `provider/model` selection resolves from layered `models.json`. OpenAI Responses providers map to native `model_provider`, `base_url`, `env_key`, header, wire API, and `-m` overrides. Codex 0.147.0 rejects a `chat` wire API, so `openai-completions` warns and omits the target instead of reusing its model ID against Codex's default endpoint. Thinking, tools, skills, subagents, plugins, and prompt templates remain unsupported and warn when selected.
