...
- **Credentials and workspace trust** — before launch, Outfitter copies `~/.claude/.credentials.json` to the temporary root as `.credentials.json` with mode `0600`. The projected `.claude.json` contains `oauthAccount` when that key is present in durable `~/.claude.json`. It also contains `projects[<cwd>].hasTrustDialogAccepted: true` only when that exact accepted trust decision already exists in durable `~/.claude.json`; other projects and unrelated machine state are not copied. After any successful or failed launch, a `.credentials.json` changed by the run is copied back and `oauthAccount` is atomically merged into durable `.claude.json` without replacing unrelated keys. Trust accepted inside an Outfitter session is discarded, so a workspace that has never been trusted by native Claude prompts again on every run. Claude's MCP OAuth token layout inside `.claude.json` is not a documented stable schema, so OAuth-backed MCP servers must be re-authorized each run.
