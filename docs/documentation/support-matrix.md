...
- **Extensions (Pi only)** — `extensions:` names Pi extension packages (`npm:`/`git:`) or existing local file/directory paths (`./`, `../`, `~/`, or absolute). Outfitter installs remote packages into its cache and passes resolved local paths directly to Pi. Codex and Claude Code launches load none of them; this is a property of the element, not a gap a user can close, so it produces no warning and does not fail under `--strict`.
