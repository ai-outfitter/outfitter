...
- **MCP servers** — Outfitter sets `PI_MCP_CONFIG_MODE=exclusive`, so compatible MCP extensions load only the servers selected into the temporary Pi agent directory. User-global, project-local, package, plugin, and imported host MCP servers do not expand the composed selection.
- **npm extensions** — `npm:`-declared extensions install into Outfitter's extension cache and load with `--extension`; their manifest entry files are also merged into the generated `settings.json` `extensions:` array, so fresh loaders (pi-subagents child sessions, SDK sessions) inherit the same set. Pi dedupes the identical paths across the flag and the settings array, so nothing loads twice. The cache also holds the extension's non-optional peer dependencies (installed with npm into the cache's npm root when missing and online), and bare `npm:<name>` specifiers install the registry's current release exactly; an explicit range whose current release it excludes warns. Satisfied cache hits stay fully offline.
