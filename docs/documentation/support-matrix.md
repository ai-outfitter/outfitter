...
- **Credentials and workspace trust** — before launch, Outfitter copies `~/.claude/.credentials.json` to the temporary root as `.credentials.json` with mode `0600`. The projected `.claude.json` contains only `oauthAccount` (when present) and `projects[<cwd>].hasTrustDialogAccepted: true` when that exact accepted trust decision already exists in durable `~/.claude.json`; other projects and unrelated machine state are not copied. After any successful or failed launch, `.credentials.json` is copied back and `oauthAccount` is atomically merged into durable `.claude.json` without replacing unrelated keys. Claude's MCP OAuth token layout inside `.claude.json` is not a documented stable schema, so MCP OAuth tokens written only there are not currently copied back.
