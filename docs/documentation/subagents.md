...
| **CI-backed**      | A [GitHub Actions run](./actions.md): dispatch a workflow that does the work, or **assignment-as-trigger** — assign the issue/PR to a machine account whose events start the workflow (runs on hosted or self-hosted runners) | A comment or PR from the run, plus a session-transcript artifact | Work that should run asynchronously with its own token scope and an auditable trail |
