...
- **Workflow-auditable** — workflow exports preserve canonical YAML, carry every used output-type schema under `output-types/`, and record every nested workflow, its resolved typed `outputs`, agent composition, file hash, and winning source in `.outfitter/workflow-composition.json`. Each `workflows[]` entry has an `outputs` object (empty when none are declared), while `outputTypes[]` records every included schema and its winning source. The manifest records only outputs that resolve, and `outfitter validate --strict` is the gate that reports declarations which do not. See [OFTR-013: Workflow Contract](../requirements/OFTR-013-workflow-contract.md).
