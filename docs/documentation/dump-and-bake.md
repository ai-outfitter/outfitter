...
- **Workflow-auditable** — workflow exports preserve canonical YAML, carry every used output-type schema under `output-types/`, and record every nested workflow, its resolved typed `outputs`, agent composition, file hash, and winning source in `.outfitter/workflow-composition.json`. Each `workflows[]` entry has an `outputs` object (empty when none are declared), and every resolved output records `schema` after `type`. Each `outputTypes[]` entry is `{ slug, id, sha256, source: { layer, path } }`, where `id` is the schema's canonical `$id` and `sha256` hashes its exact stored bytes; entries are sorted by slug. The manifest records only outputs that resolve, and `outfitter validate --strict` is the gate that reports declarations which do not. See [OFTR-013: Workflow Contract](../requirements/OFTR-013-workflow-contract.md).
