...
Flat profiles are easy to scan, diff, and copy between setup repositories. Generated system prompt exports for flat profiles are written beside the flat file as `<profile-id>.generated-system-prompt.md` when `profile_export` is enabled.
...
Directory profiles keep bundled resources close to the profile that references them. Generated system prompt exports for directory profiles are written as `generated-system-prompt.md` inside the profile directory when `profile_export` is enabled.
...
With `profile_export: true`, the selected project directory profile can write `generated-system-prompt.md` beside `profile.yml`. The artifact reflects the adapter selected for the launch. On a Pi launch it is overwritten at session start with the full runtime prompt from `ctx.getSystemPrompt()`. On a Claude Code launch — which has no runtime prompt read-back — it records the composed prompt inputs Outfitter supplies (effective `system_prompt` and resolved `append_system_prompt` includes), not Claude's full base prompt. Either way no launch flag beyond selecting the adapter is required.
For a Pi launch, the generated prompt would show the composed prompt inputs like this:
