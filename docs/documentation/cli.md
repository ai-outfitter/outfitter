...
| -------- | ------------------------------------------------------------------------------------------ |
| `[kind]` | Optional filter: `agents`, `skills`, `knowledge`, `commands`, `workflows`, `output-types`. |
...
Workflow dumps are non-executable configuration bundles. They contain the canonical workflow YAML,
composed agent resources, the schemas used by the closure under `output-types/`, and a
hash/provenance manifest whose `workflows[]` entries record resolved `outputs` and whose
`outputTypes[]` entries record each schema's winning source. A workflow dump refuses an existing
destination instead of replacing user files.
