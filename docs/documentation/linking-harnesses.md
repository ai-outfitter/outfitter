# Linking into Claude Code and Codex

`outfitter run` projects a composition into a temporary directory for one launch. `outfitter link` is the persistent form: it places managed links to the composed tree inside a harness's own home, so a plain `claude` or `codex` session carries your skills, agents, commands, shared context, and MCP servers with no Outfitter in the loop.
...
| Tree resource                               | Claude Code home                       | Codex home          | How        |
| ------------------------------------------- | -------------------------------------- | ------------------- | ---------- |
| Winning tree-root `agents.md`               | `CLAUDE.md`                            | `AGENTS.md`         | symlink    |
| Skill `skills/<slug>/`                      | `skills/<slug>`                        | `skills/<slug>`     | symlink    |
| Composed agent (scoped agent and delegates) | `agents/<slug>.md`                     | — (warning)         | generated  |
...
- `settings.json` — permissions, model, and hooks remain harness-native; see [Hooks](./hooks.md)
...
Removes exactly the manifest entries — symlinks, generated agent files, and MCP servers it added (through `mcp remove`) — and forgets them. Managed containers (`skills/`, `agents/`, `commands/`, `prompts/`) are removed when they are left empty. Anything unmanaged beside them is untouched.
