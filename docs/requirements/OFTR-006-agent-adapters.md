...
14. After a Claude Code launch exits or throws, the adapter MUST copy a projected `.credentials.json` back to `~/.claude/.credentials.json` with mode `0600` when the run changed it.
...
16. Copy-back of Claude MCP OAuth tokens is DEFERRED pending a stable, documented `.claude.json` MCP token schema; the adapter MUST NOT merge projected MCP OAuth token state back until that schema is available and the behavior is specified.
