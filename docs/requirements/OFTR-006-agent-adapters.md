...
22. The pi adapter MUST overlay settings-layer `agent_defaults.pi_overlay` directories into the temporary `PI_CODING_AGENT_DIR` below the per-agent `pi/` overlay (item 17) and above generated defaults, with higher-precedence settings layers replacing matching paths, without following symlinks or projecting the overlay into non-Pi harnesses. A non-Pi harness MUST report a configured settings-layer overlay as an unsupported control (fatal under `--strict`), and a declared overlay directory that is missing, is not a directory, or is a symlink MUST be reported as a warning (fatal under `--strict`) instead of being silently skipped.
