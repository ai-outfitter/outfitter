...
- Outfitter does not do generic post-run copy-back or JSON/YAML merge-back. The Claude adapter has
  a narrow credential bridge as the one exception: `.credentials.json` is copied because the
  projection root is ephemeral, and `oauthAccount` is merged because its durable home
  `~/.claude.json` sits outside `~/.claude` and holds machine-local state that must not be copied
  wholesale.
