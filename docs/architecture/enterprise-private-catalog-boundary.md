...
This document defines the enterprise/private catalog boundary for Outfitter. Private catalog support is an informational commercial-governance control, not DRM.
...
Outfitter does not collect, echo, persist, synthesize, or validate credentials for private catalogs. Future private repository access may rely on the user's existing local Git configuration, such as SSH agents, credential helpers, netrc, or CI-provided Git configuration, because current repository access delegates to `git`.

`~/.agents/settings.yml` is the source of truth for private catalog enablement:
...
Enable private catalogs in ~/.agents/settings.yml? [y/N]
```

Non-interactive CLI setup/sync skips that source with this informational copy:

```text
info: Private GitHub catalog detected: OWNER/REPO. Enable enterprise.private_catalogs in ~/.agents/settings.yml after reviewing code/enterprise/LICENSE or your enterprise agreement.
```
...
- No change to resource resolution, settings merge precedence, or composition assembly.
