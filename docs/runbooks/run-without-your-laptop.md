...
**Start with issue triage, then pull-request review.** Both are cheap to run in Actions: the trigger is a forge event the platform already delivers, and the credential is the job's own `GITHUB_TOKEN`. Both also have a prerequisite that decides how well they work. Add issue templates at `.github/ISSUE_TEMPLATE/` before triage, so the agent reads a typed form rather than guessing at prose; add `CODEOWNERS` before review, so the agent respects routing the forge already applies. `ai-outfitter/actions` ships both as examples — [`issue-triage-github-models.yml`](https://github.com/ai-outfitter/actions/blob/main/examples/issue-triage-github-models.yml), which needs no API key, and [`review-undrafted-pr.yml`](https://github.com/ai-outfitter/actions/blob/main/examples/review-undrafted-pr.yml).
