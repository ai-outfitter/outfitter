...
The exact stable schema is governed by the requirements in `requirements/` and the JSON Schema files in `schemas/`, which are still expected to evolve with implementation.
