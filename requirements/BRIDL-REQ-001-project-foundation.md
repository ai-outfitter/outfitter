# BRIDL-REQ-001: Project Foundation
...
### BRIDL-REQ-001.1: Runtime, Package Manager, and Language
...
### BRIDL-REQ-001.4: Persisted File Format Policy

1. User-editable persisted Bridl configuration MUST use YAML instead of JSON unless the file is a JSON Schema.
2. Every user-editable YAML file format that Bridl reads MUST have a corresponding JSON Schema.
3. Bridl MUST validate YAML files against their JSON Schemas anywhere those files are read.
4. JSON Schema files MAY use JSON because schemas are tooling-facing validation artifacts.

### BRIDL-REQ-001.5: Initial Dependency Set
