...
Outfitter release publishing prepares package metadata from a GitHub release tag and publishes the `outfitter` npm package through GitHub Actions.
...
6. The npm release workflow MUST publish the public `outfitter` package to the npm registry using an npm token secret.
